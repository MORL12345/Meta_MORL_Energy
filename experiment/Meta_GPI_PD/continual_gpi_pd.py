...
class GPIPD(MOPolicy, MOAgent):
    ...
    def train_iteration(
        ...
        obs, info = self.env.reset(hour_scope=hour_scope)
        episode_steps = 0
        for _ in range(1, total_timesteps + 1):
            ...
            if eval_env is not None and self.log and self.global_step % eval_freq == 0:
                ...
                mean_u, mean_rew_v = evaluate_model(model=self,
                                                    hour_scope=self.env.hour_scope,
                                                    reward_dim=reward_dim,
                                                    granularity=granularity,
                                                    eval_env=eval_env)

                wandb.log({'expected utility': mean_u,
                           'expected bill return': mean_rew_v[0],
                           'expected comfort return': mean_rew_v[1]}, step=self.global_step)
                print(f"@episode:{self.num_episodes}\t"
                      f"DQN-agent @ mean_u:{mean_u} -- mean[bill,comfort]:{mean_rew_v}\t "
                      f"were issued for 30 days")
    ...
    def train(
        ...
        for iter in range(1, max_iter + 1):
            ...
            # )
            # print(f"@iteration:{self.global_step / timesteps_per_iter} - mean_utility:{mean_gpi_returns_test_tasks}")
    ...
    def continual_rl_train(self, seed=42, pretrained_model_path=None, eval_env=None):
        """
        ...
        print("CRL starts")
        anomaly_weeks = [4, 6, 8, 10, 12, 16, 23, 29, 33, 38, 51]
        ...
        for anomaly_week_idx in range(len(anomaly_weeks)):
            ...
            if hour_scope[0] <= 50:
                self.q_nets = th.load("agent_model/GPI_PD/23/models/39600.pth", map_location=self.device)
                print("loaded")
            else:

                self.global_step = 0
                # clear up the replay buffer to remove the old experience
                self.replay_buffer = PrioritizedReplayBuffer(
                    self.observation_shape, 1, rew_dim=self.reward_dim, max_size=int(1e6), action_dtype=np.uint8
                )
                self.env.hour_scope = hour_scope
                # train with the first 4 weeks data
                self.train(
            ...
            for i in range(len(eval_weights)):
                w = eval_weights[i]
                if hour_scope[0] > 0:
                    eval_hour_scope = [week_hour_conversion(anomaly_weeks[anomaly_week_idx - 1]),
                                       week_hour_conversion(week)]
                    cumulative_rews = self.play_a_episode_(hour_scope=eval_hour_scope,
                                                           weight=w,
                                                           eval_env=eval_env)
        ...
        wandb.init(project='MO_Energy', entity='junlinlu16', name="continual_rl_train_" + str(seed))
        wandb.log({'CL_EU': CL_EU/len(eval_weights)})
        return solutions
    ...
    def play_a_episode_(self, eval_env, weight, hour_scope):
